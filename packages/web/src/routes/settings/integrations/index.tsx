...
const toItems = (opts: ToItemsOpts): IntegrationItem[] => {
  const { identities, repos, contexts, instances, connections, identityMap } = opts;
  return [
    ...identities.map(
      (i): IntegrationItem => ({
        id: i.id,
    ...
    ...repos.map(
      (r): IntegrationItem => ({
        ...
        detail: r.cloneUrl,
        tags:
          r.sshIdentityId && identityMap.has(r.sshIdentityId)
    ...
    ...connections.map(
      (c): IntegrationItem => ({
        ...
        tags:
          c.sshIdentityId && identityMap.has(c.sshIdentityId)
...
type EmptyStateProps = { onAdd: (type: IntegrationType) => void };

const EmptyState = ({ onAdd }: EmptyStateProps): React.ReactElement => (
    ...
    <div className="flex flex-wrap justify-center gap-2">
      {(Object.entries(integrationTypes) as [IntegrationType, (typeof integrationTypes)[IntegrationType]][]).map(
        ([type, config]) => {
          ...
          return (
            <Button key={type} variant="secondary" size="sm" icon={Icon} onClick={() => onAdd(type)}>
              {config.singular}
            </Button>
          );
        },
...
type GroupedListProps = {
  groups: Record<IntegrationType, IntegrationItem[]>;
  onAdd: (type: IntegrationType) => void;
  onEdit: (type: IntegrationType, id: string) => void;
};

const GroupedList = ({ groups, onAdd, onEdit }: GroupedListProps): React.ReactElement => {
  let globalIndex = 0;
  return (
    <>
      {(Object.entries(groups) as [IntegrationType, IntegrationItem[]][]).map(([type, groupItems], sectionIndex) => (
        <div key={type}>
          <SectionHeader type={type} delay={sectionIndex * 0.08} onAdd={() => onAdd(type)} />
          {groupItems.length > 0 && (
...
const IntegrationsIndex = (): React.ReactElement => {
  ...
  const handleAdd = (type: IntegrationType): void => {
    navigate({ to: addRoutes[type] });
  };

  const handleEdit = (type: IntegrationType, id: string): void => {
    navigate({ to: editRoutes[type](id) });
  };
  ...
  return (
    ...
    >
      {!hasAny ? (
        ...
        <GroupedList groups={groups} onAdd={handleAdd} onEdit={handleEdit} />
      )}
    </SettingsShell>
  );
};
