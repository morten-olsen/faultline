...
const StageTransition = ({ from, to, reason }: { from: string; to: string; reason: string }): React.ReactElement => (
  ...
  >
    ...
    <div style={{ fontSize: '0.75rem', textAlign: 'center', maxWidth: 360, lineHeight: 1.5 }}>{reason}</div>
  </motion.div>
);
...
const Chapter = ({ number, title, desc, color, icon }: ChapterProps): React.ReactElement => {
  ...
  return (
    <motion.div {...fadeUp} style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        ...
        <IconComponent size={16} style={{ color, opacity: 0.8 }} />
...
const HomeRow = ({
  ...
  needsYou,
  index = 0,
}: HomeRowProps): React.ReactElement => {
  const IconComponent = icon;
  return (
    <motion.div
      {...stagger(index, 0.15)}
      style={{
        ...
        borderRadius: 10,
        border: needsYou ? '1px solid rgba(245,158,11,0.1)' : '1px solid rgba(255,255,255,0.04)',
      }}
    >
      <div style={{ marginTop: 2 }}>
        <IconComponent size={14} style={{ color: iconColor, opacity: 0.8 }} />
      </div>
  ...
  <div style={{ maxWidth: 640, margin: '0 auto', color: '#e8e8e8' }}>
    <motion.div {...fadeUp}>
      ...
      <p style={{ color: '#a3a3a3', fontSize: '0.875rem', lineHeight: 1.7, marginBottom: 8, maxWidth: 520 }}>
        Follow one issue — memory pressure on a Kubernetes node — from the moment the system detects it to the moment
        it's resolved. At each stage, see how the interface adapts: what the home page shows, what the detail page
        reveals, and what the user experiences.
      </p>
      <p style={{ color: '#5a5a5a', fontSize: '0.8125rem', lineHeight: 1.6, marginBottom: 32 }}>
        This is the story of how Faultline works. Every component, every color choice, every animation exists to serve
        this flow.
      </p>
    </motion.div>
    ...
    <motion.div {...stagger(0, 0.15)} style={{ maxWidth: 420 }}>
      <AgentActivity status="complete" label="Checked allocations — ran 4 queries in 2.3s" onExpand={() => undefined} />
    </motion.div>
    <motion.div
      {...stagger(0, 0.2)}
      style={{
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        Every timeline entry that involved agent work shows an{' '}
        <strong style={{ color: '#e8e8e8' }}>AgentActivity</strong> indicator beneath it. Tap the chevron to open the
        full transcript — every{' '}
        <code
          style={{ fontSize: '0.75rem', background: 'rgba(255,255,255,0.06)', padding: '1px 5px', borderRadius: 4 }}
        >
          kubectl
        </code>{' '}
        command, every piece of reasoning, every decision. Most users won't look — but they can.
      </p>
    </motion.div>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        Not every plan needs approval. Safe, reversible actions (moving low-priority pods) proceed automatically.
        Disruptive plans (evicting home-assistant) pause here and flag{' '}
        <strong style={{ color: '#e8e8e8' }}>Needs you</strong>. The system knows the difference.
      </p>
    </motion.div>
    ...
    <motion.div {...stagger(0, 0.15)} style={{ marginLeft: 28, marginTop: -8, marginBottom: 16, maxWidth: 380 }}>
      <AgentActivity status="complete" label="Rebalanced 3 pods — 6 commands, 4.2s" onExpand={() => undefined} />
    </motion.div>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        Regressions are honest. The system doesn't pretend a problem is solved when it isn't. The issue reappears in the
        pipeline with a <span style={{ color: '#fbbf24' }}>Regressed</span> badge — visible proof that the system tried,
        failed, and is trying again. It re-enters at Investigation to analyze what went wrong.
      </p>
    </motion.div>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        <strong style={{ color: '#e8e8e8' }}>This is not an error.</strong> The system is asking, not alarming. The
        amber tone and <Hand size={11} style={{ color: '#fbbf24', display: 'inline', verticalAlign: 'middle' }} /> hand
        icon say "hey, when you get a moment" — not "CRITICAL ALERT." The user on the metro can approve with their thumb
        and put their phone away.
      </p>
    </motion.div>
    ...
    >
      ...
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 12px' }}>
        Memory has been above 80% for over an hour despite rebalancing. The best fix is to evict home-assistant
        temporarily — your smart home would be offline for about 3 minutes.
      </p>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        The agent indicator turns <strong style={{ color: '#fbbf24' }}>amber</strong> when waiting — same color language
        as "Needs you" everywhere else. The stop button lets you halt the agent entirely. The chevron opens the full
        transcript so you can see *why* this decision was reached before approving.
      </p>
    </motion.div>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        The rejection reason is carried into the next investigation as context. The agent sees "The previous plan was
        rejected because: tried this before, didn't work" and adapts its approach. The{' '}
        <strong style={{ color: '#e8e8e8' }}>user-action</strong> entry in the timeline preserves the human's decision
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          ...
          <span style={{ fontSize: '0.6875rem', color: '#5a5a5a', fontFamily: '"Geist Mono Variable", monospace' }}>
            50m
          </span>
        </div>
    ...
    >
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: 0 }}>
        Lower visual weight. The title uses secondary color. The checkmark is translucent. Everything says: "this is
        history, not action." Over weeks and months, the resolved log becomes the reason the user stops checking —
        they've seen enough to trust the system.
      </p>
    </motion.div>
    ...
    <motion.div {...stagger(0, 0.3)} style={{ margin: '48px 0 0', borderTop: '1px solid #1c1c1c', paddingTop: 32 }}>
      ...
      <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, marginBottom: 20, maxWidth: 480 }}>
        Every step the system took is visible. The user can trace the full story from detection to resolution. Nothing
        is hidden. This is how trust is built.
      </p>
    </motion.div>
      ...
      {[
        {
          ...
          time: '14:52',
          body: 'Memory stable for 10 minutes. Closing.',
        },
        {
          kind: 'outcome' as const,
          status: 'success' as const,
          title: 'Memory dropped to 58%',
          time: '14:42',
          body: 'home-assistant evicted. Memory stabilized.',
        },
        {
        ...
        },
        {
          ...
          title: 'New plan: evict home-assistant to free 1.2 GB',
          time: '14:18',
          body: 'Safe options exhausted. This is the most effective remaining option.',
        },
        {
          ...
          title: 'Memory climbed back above 80%',
          time: '14:14',
          body: "First rebalance didn't hold.",
        },
        {
          kind: 'outcome' as const,
          status: 'success' as const,
          title: 'Memory dropped to 68%',
          time: '14:06',
        ...
        },
        {
          kind: 'analysis' as const,
          status: 'info' as const,
          title: 'Identified 3 movable low-priority pods',
          time: '14:03',
          body: 'prometheus-adapter, metrics-server, node-exporter. Safe to reschedule.',
        },
        {
        ...
        <motion.div key={entry.time + entry.title} {...stagger(i, 0.35)}>
          <TimelineEntry {...entry} isLast={i === arr.length - 1} />
        </motion.div>
      ))}
    </div>
  </div>
