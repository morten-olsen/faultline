...
const SpacingRow = ({ name, rem, px, usage }: SpacingRowProps): React.ReactElement => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
    ...
    <div style={{ minWidth: 56 }}>
      <span style={{ color: '#e8e8e8', fontSize: '0.8125rem', fontFamily: '"Geist Mono Variable", monospace' }}>
        {name}
      </span>
    </div>
    <span
      style={{ color: '#5a5a5a', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace', minWidth: 40 }}
    >
      {px}
    </span>
...
const SpacingStory = (): React.ReactElement => (
  <div style={{ maxWidth: 720, color: '#e8e8e8' }}>
    ...
    <p style={{ color: '#a3a3a3', fontSize: '0.875rem', lineHeight: 1.7, margin: '0 0 2.5rem', maxWidth: 560 }}>
      Generous spacing is how the interface breathes. The calm comes from the room you give things — a checkmark
      centered on a wide page feels peaceful; the same checkmark crammed between stats feels like another data point.
      Default to spacious.
    </p>
    ...
    <h3 style={{ color: '#e8e8e8', fontSize: '1.125rem', fontWeight: 500, margin: '0 0 0.5rem' }}>Border Radius</h3>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.7, margin: '0 0 1.5rem', maxWidth: 520 }}>
      Consistently rounded — modern but not bubbly. Buttons and inputs use `rounded-lg` (12px). Cards and containers use
      `rounded-xl` (16px). Badges and pills use `rounded-full`.
    </p>
    ...
    <h3 style={{ color: '#e8e8e8', fontSize: '1.125rem', fontWeight: 500, margin: '0 0 0.5rem' }}>The narrow column</h3>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.7, margin: '0 0 1.5rem', maxWidth: 520 }}>
      Every page uses <code style={{ color: '#60a5fa', fontSize: '0.75rem' }}>max-w-lg</code> (512px). On desktop it
      doesn't fill the screen, and that's the point. A wide layout invites complexity. A narrow layout forces
      simplicity. It also means the mobile experience is identical — no responsive breakpoints, no degraded version.
    </p>
    ...
    <div style={{ position: 'relative', margin: '1.5rem 0', padding: '1rem 0' }}>
      ...
      >
        <span style={{ color: '#5a5a5a', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace' }}>
          max-w-lg · 512px
        </span>
      </div>
    </div>
