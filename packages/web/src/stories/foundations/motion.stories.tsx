...
import { CircleCheck, Eye, Wrench, Hand, Search, RotateCcw } from 'lucide-react'
...
const SectionLabel = ({ children }: { children: React.ReactNode }): React.ReactElement => (
  <div style={{
    color: '#5a5a5a', fontSize: '0.6875rem', fontWeight: 500,
    textTransform: 'uppercase', letterSpacing: '0.06em',
    marginBottom: 12, marginTop: 48,
  }}>
    {children}
  </div>
)
...
const ReplayButton = ({ onReplay }: { onReplay: () => void }): React.ReactElement => (
  <button
    ...
    style={{
      ...
      color: '#a3a3a3', fontSize: '0.75rem', cursor: 'pointer',
    }}
  >
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        {([
          ...
          <div key={`${label}-${key}`} style={{ flex: 1 }}>
            ...
            <div style={{ color: '#e8e8e8', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace' }}>{label}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        ...
        ] as const).map(({ label, duration, usage, color }) => (
          <div key={`${label}-${key}`} style={{ flex: 1 }}>
            <div style={{
              height: 80, borderRadius: 10,
              background: '#141414', overflow: 'hidden',
            ...
            <div style={{ color: '#e8e8e8', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace', marginTop: 8 }}>{label}</div>
  ...
  return (
    <div>
      ...
      <div style={{ background: '#141414', borderRadius: 12, padding: 16, maxWidth: 420 }}>
        {rows.map(({ icon: Icon, color, title, tag }, i) => (
          <motion.div
            key={`${title}-${key}`}
      ...
      <div style={{ marginTop: 12, display: 'flex', gap: 16 }}>
        <div>
          <span style={{ color: '#e8e8e8', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace' }}>50ms</span>
          <span style={{ color: '#5a5a5a', fontSize: '0.6875rem', marginLeft: 6 }}>stagger delay between rows</span>
        </div>
        <div>
          <span style={{ color: '#e8e8e8', fontSize: '0.75rem', fontFamily: '"Geist Mono Variable", monospace' }}>8px</span>
  ...
  return (
    <div>
        ...
        <AnimatePresence>
          {visible && (
            ...
            >
              ...
              <div>
                <div style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#e8e8e8' }}>Approve evicting home-assistant?</div>
                <div style={{ fontSize: '0.75rem', color: '#5a5a5a', marginTop: 2 }}>Smart home offline for ~3 minutes.</div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div style={{ marginTop: 8, color: '#5a5a5a', fontSize: '0.6875rem' }}>
        Enter: fade + slide up (10px) · Exit: fade + slide up (8px) — exits are slightly faster and shorter than entrances.
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 12, color: '#5a5a5a', fontSize: '0.6875rem', maxWidth: 380, lineHeight: 1.6 }}>
        The status line arrives first (0ms), subtitle follows (80ms), the needs-you card
        settles in (150ms), then section label (350ms) and rows stagger (400ms+). The user's
        eye follows the hierarchy naturally.
      </div>
    </div>
  )
}
...
const MotionStory = (): React.ReactElement => (
  <div style={{ maxWidth: 720, color: '#e8e8e8' }}>
    ...
    <p style={{ color: '#a3a3a3', fontSize: '0.875rem', lineHeight: 1.7, margin: '0 0 1rem', maxWidth: 560 }}>
      Things don't just appear — they enter. And when they leave, they exit.
      Motion is not decoration; it is narrative. A card that fades up from below
      <em> arrives</em>. A row that staggers in after its siblings tells the user
      "there's a sequence here."
    </p>
    <p style={{ color: '#5a5a5a', fontSize: '0.8125rem', lineHeight: 1.7, margin: '0 0 2rem', maxWidth: 560 }}>
      We use <a href="https://motion.dev" style={{ color: '#60a5fa', textDecoration: 'none' }}>Motion</a> (framer-motion)
      for orchestrated animations. CSS transitions handle hover and focus states.
    </p>

    <SectionLabel>Easing</SectionLabel>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
      We use one easing curve for almost everything: a custom <code style={{ color: '#60a5fa', fontSize: '0.6875rem' }}>easeOut</code> that
      starts fast and lands gently. Content arrives with momentum and settles into place.
    </p>
    <EasingDemo />

    <SectionLabel>Timing</SectionLabel>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
      Fast enough to feel responsive, slow enough to be perceived. The range is
      120ms–400ms. Nothing slower — the interface should feel like content settling,
      not a slideshow.
    </p>
    <TimingDemo />
    ...
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
      When multiple elements appear, cascade them with short delays (30–50ms between siblings).
      The user's eye follows the rhythm and absorbs the hierarchy without conscious effort.
      The stagger reveals structure — it says "these are related, and they have an order."
    </p>
    <StaggerDemo />

    <SectionLabel>Enter & Exit</SectionLabel>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
      Every element that appears should have a moment — a subtle fade and short slide.
      Removed elements exit with the same care. The user should feel continuity, not disruption.
      Exits are slightly faster and travel less distance than entrances.
    </p>
    <EnterExitDemo />

    <SectionLabel>Hierarchy through timing</SectionLabel>
    <p style={{ color: '#a3a3a3', fontSize: '0.8125rem', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
      Animation delay establishes what matters most. The status line arrives first because
      that's the first question ("am I good?"). The needs-you card enters second because that's
      the next question ("do I need to do anything?"). Everything else follows. The motion
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {[
        { rule: 'Enter, don\'t snap.', desc: 'Every element gets a moment — subtle fade, short slide. Enough to feel intentional, not enough to slow the user down.' },
        { rule: 'Stagger to show structure.', desc: 'Cascade siblings with 30–50ms delays. The eye follows the rhythm and absorbs the hierarchy.' },
        { rule: 'Keep it quiet.', desc: '200–400ms duration. Gentle easeOut. 8–12px travel. Nothing bounces, nothing overshoots, nothing calls attention to itself.' },
        { rule: 'Exit with care.', desc: 'Removed elements fade and slide away. Slightly faster than entrance. The user feels continuity, not jump cuts.' },
        { rule: 'Hierarchy through delay.', desc: 'The most important thing arrives first. Animation order mirrors the information hierarchy.' },
      ].map(({ rule, desc }) => (
