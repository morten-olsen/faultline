...
const nasHealthSteps: StepData[] = [
  ...
  },
  {
    ...
    kind: 'message',
    title:
      'All three disks are healthy — zero reallocated sectors, temperatures normal (33–35°C). The ZFS pool is online with no errors. Last scrub completed clean.',
  },
];
    ...
    <div className="space-y-4">
      ...
      <motion.div {...stagger(1, 0.1)}>
        <ChatBubble role="assistant" timestamp="just now">
          All clear. Three nodes healthy, all workloads running. Memory settled at 68–74% across the cluster after this
          morning's rebalance. No open issues.
        </ChatBubble>
      </motion.div>
    </div>
  </ChatLayout>
...
const DeeperQuestion = (): React.ReactElement => (
  <ChatLayout composer={<ChatComposer />}>
    <div className="space-y-4">
      <motion.div {...stagger(0, 0.1)}>
        <ChatBubble role="user">What happened with CoreDNS this morning?</ChatBubble>
      </motion.div>
      <motion.div {...stagger(1, 0.1)}>
        <ChatBubble role="assistant">
          A ConfigMap update at 13:58 broke the forward plugin — it removed the upstream nameserver block, so CoreDNS
          couldn't resolve external DNS. It crashed three times before I caught it.
        </ChatBubble>
      </motion.div>
      <motion.div {...stagger(2, 0.1)}>
        <ChatBubble role="assistant">
          I rolled back to the previous stable Corefile and confirmed DNS was healthy at 3ms latency. Took about two
          minutes end to end. The bad ConfigMap is still in the rollout history if you want to see what changed.
        </ChatBubble>
      </motion.div>
      <motion.div {...stagger(3, 0.1)}>
        <ChatBubble role="user">Who applied that ConfigMap change?</ChatBubble>
      </motion.div>
      <motion.div {...stagger(4, 0.1)}>
        <ChatBubble role="assistant">
          Checking the audit log — it was applied via kubectl by the CI pipeline at 13:58. Looks like it came from a
          Flux reconciliation, not a manual edit.
        </ChatBubble>
      </motion.div>
    </div>
  </ChatLayout>
...
const AdHocTask = (): React.ReactElement => (
  <ChatLayout composer={<ChatComposer />}>
    <div className="space-y-4">
      <motion.div {...stagger(0, 0.1)}>
        <ChatBubble role="user">Can you check the disk health on the NAS?</ChatBubble>
      </motion.div>
      <motion.div {...stagger(1, 0.1)}>
        <ChatBubble role="assistant">Sure, let me take a look.</ChatBubble>
      </motion.div>
      <motion.div {...stagger(2, 0.1)}>
...
const AgentWorking = (): React.ReactElement => (
  <ChatLayout composer={<ChatComposer disabled placeholder="Agent is working…" />}>
    <div className="space-y-4">
      <motion.div {...stagger(0, 0.1)}>
        <ChatBubble role="user">The API has been slow today. Can you look into it?</ChatBubble>
      </motion.div>
      <motion.div {...stagger(1, 0.1)}>
        <ChatBubble role="assistant">On it — checking traefik and the upstream services.</ChatBubble>
      </motion.div>
      <motion.div {...stagger(2, 0.1)}>
        <AgentStepsView steps={apiLatencySteps} isRunning={true} />
      </motion.div>
      <motion.div {...stagger(3, 0.1)} className="ml-8.5">
        <AgentActivity status="working" label="Investigating API latency" elapsed="12s" />
      </motion.div>
    </div>
  </ChatLayout>
);
