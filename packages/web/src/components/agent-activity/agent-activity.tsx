    ...
    `}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          ...
          <span className={`text-xs flex-shrink-0 ${status === 'waiting' ? 'text-amber-400/80' : 'text-text-muted/50'}`}>
            {statusLabel[status]}
          </span>
        </div>
      </div>
      ...
      {/* Elapsed */}
      {elapsed && (
        <span className="text-xs text-text-muted/40 font-mono flex-shrink-0">{elapsed}</span>
      )}
      ...
      {isActive && onStop && (
        ...
        >
          <Square size={12} className="text-text-muted/50 group-hover:text-red-400 transition-colors" fill="currentColor" />
        </button>
      )}
