...
const ClientProvider = ({ url, children }: ClientProviderProps): React.ReactElement => {
  ...
  return <ClientContext.Provider value={{ client, connected }}>{children}</ClientContext.Provider>;
};
...
const useClientStatus = (): { connected: boolean } => {
  const ctx = useContext(ClientContext);
  if (!ctx) {
    throw new Error('useClientStatus must be used within a ClientProvider');
  }
