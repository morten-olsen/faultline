...
import { transition, issueEventSchema, InvalidTransitionError } from './issues.machine.js';
...
const fail = (stage: IssueStage, event: IssueEvent) => {
  const result = transition(stage, event);
  if (result.ok) {
    throw new Error(`Expected failure, got stage: ${result.effect.stage}`);
  }
...
describe('event data flows to issue updates', () => {
  ...
  it('ENTER_MONITORING computes monitoring schedule from duration and interval', () => {
    ...
    const until = new Date(updates?.monitor_until as string).getTime();
    ...
    const next = new Date(updates?.monitor_next_check_at as string).getTime();
...
describe('terminal states', () => {
  ...
  it.each(nonReopenEvents.map((e) => [e.type, e] as const))('resolved rejects %s', (_name, event) => {
    const result = transition('resolved', event);
    expect(result.ok).toBe(false);
  });

  it.each(allEvents.map((e) => [e.type, e] as const))('ignored rejects %s', (_name, event) => {
