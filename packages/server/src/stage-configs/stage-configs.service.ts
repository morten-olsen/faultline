...
class StageConfigService {
  ...
  upsert = async (input: UpsertStageConfigInput): Promise<ParsedStageConfig> => {
    ...
    const values = {
      ...
      stage: input.stage,
      allowed_kube_contexts: input.allowedKubeContexts
        ? JSON.stringify(input.allowedKubeContexts)
        : null,
      allowed_ssh_connections: input.allowedSshConnections
        ? JSON.stringify(input.allowedSshConnections)
        : null,
      allowed_git_repos: input.allowedGitRepos
        ? JSON.stringify(input.allowedGitRepos)
        : null,
      allowed_argocd_instances: input.allowedArgocdInstances
        ? JSON.stringify(input.allowedArgocdInstances)
        : null,
