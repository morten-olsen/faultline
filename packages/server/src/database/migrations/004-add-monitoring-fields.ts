...
const up = async (db: Kysely<unknown>): Promise<void> => {
  await db.schema.alterTable('issues').addColumn('monitor_plan', 'text').execute();

  await db.schema.alterTable('issues').addColumn('monitor_interval_minutes', 'integer').execute();

  await db.schema.alterTable('issues').addColumn('monitor_next_check_at', 'text').execute();

  await db.schema.alterTable('issues').addColumn('monitor_until', 'text').execute();

  await db.schema
    .alterTable('issues')
    .addColumn('monitor_checks_completed', 'integer', (col) => col.defaultTo(0))
    .execute();

  await db.schema.alterTable('approvals').addColumn('decision_reason', 'text').execute();
};
