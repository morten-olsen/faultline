...
const up = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema
    .createIndex("idx_issues_stage")
    .on("issues")
    .column("stage")
    .execute();

  await db.schema
    .createIndex("idx_issues_needs_you")
    .on("issues")
    .column("needs_you")
    .execute();
  ...
  await db.schema
    .createIndex("idx_issue_resources_issue_id")
    .on("issue_resources")
    .column("issue_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("id", "text", (col) => col.primaryKey())
    .addColumn("source_issue_id", "text", (col) =>
      col.notNull().references("issues.id"),
    )
    .addColumn("target_issue_id", "text", (col) =>
      col.notNull().references("issues.id"),
    )
  ...
  await db.schema
    .createIndex("idx_issue_relations_source")
    .on("issue_relations")
    .column("source_issue_id")
    .execute();

  await db.schema
    .createIndex("idx_issue_relations_target")
    .on("issue_relations")
  ...
  await db.schema
    .createIndex("idx_approvals_issue_id")
    .on("approvals")
    .column("issue_id")
    .execute();

  await db.schema
    .createIndex("idx_approvals_status")
    .on("approvals")
    .column("status")
    .execute();
