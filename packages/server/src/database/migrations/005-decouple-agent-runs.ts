...
const up = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema
    ...
    .addPrimaryKeyConstraint('pk_issue_agent_runs', ['issue_id', 'agent_loop_id'])
    .execute();

  await db.schema.createIndex('idx_issue_agent_runs_issue_id').on('issue_agent_runs').column('issue_id').execute();
  ...
  await sql`INSERT INTO agent_loops (id, title, status, started_at, finished_at) SELECT id, title, status, started_at, finished_at FROM agent_loops_old`.execute(
    db,
  );
...
const down = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await sql`INSERT INTO agent_loops (id, issue_id, title, status, started_at, finished_at) SELECT al.id, iar.issue_id, al.title, al.status, al.started_at, al.finished_at FROM agent_loops_new al JOIN issue_agent_runs iar ON iar.agent_loop_id = al.id`.execute(
    db,
  );
