...
- Use `.js` for all TypeScript source files (backend and packages) — this matches the compiled output from `tsc`
- Use `.tsx` for files containing JSX (frontend code uses Vite's bundler resolution, which does **not** resolve `.ts` to `.tsx`)

```typescript
// ✅ Good (all tsc-compiled code — backend and packages)
import { createUser } from "./user-service.js";
import type { User } from "./types.js";

// ✅ Good (frontend code — use actual extension)
...
// ❌ Bad - .ts extension (does not match compiled output)
import { createUser } from "./user-service.ts";
```

**Rationale**:

- All backend and package code is compiled with `tsc`, which does not rewrite import extensions — `.js` must be used so imports resolve correctly against the compiled output
- Vite's bundler resolution requires exact extensions — `.ts` will not resolve to a `.tsx` file
...
logger.info("Server starting", { port: 3007 });
...
- [ ] Including `.js` extensions in imports (matches tsc output)
